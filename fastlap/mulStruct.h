...
#ifndef _mulstruc_h_included_
#define _mulstruc_h_included_
...
struct cube {
    ...
    int upnumvects;             // 0 if empty,  1 on bot level if not empty, else # nonempty kids.
    int *upnumeles;             // numeles[0] = # sngs on bot level, else number of terms in kid's expansion.
    double **upvects;           // vects[0] = sngs on bot level, else vectors of kids' expansion terms.
    ...
    double ***upmats;           // Matrices for singularity to multi or multi
                                // to multi. upmats[i] is multisize x upnumeles[i].
...
//  struct cube *rpnext;        // Reverse ptr to next cube to do precond.
    ...
    snglrty **sngs;             // Array of singularity ptrs. Only used lowest level.
    fieldpt **fpts;             // Array of field point ptrs. Only used lowest level.
    struct cube **nbrs;         // Array of ptrs to neighbors with singularities for direct comps.
    int numnbrs;                // Number of nbrs-type neighbors.
    struct cube **pnbrs;        // Array of ptrs to neighbors with field pts for precond comps.
    struct cube **qnbrs;        // Array of ptrs to neighbors with sing's for precond comps.
...
struct ssystem {
    ...
    cube **multilist;           // Array of ptrs to first cube in linked list of cubes to do multi at each level.
    cube **locallist;           // Array of ptrs to first cube in linked list of cubes to do local at each level.
    cube *directlist;           // Head of linked lst of leaf cubes w/ field points and neighbors.
